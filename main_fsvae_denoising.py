...
def train(network, trainloader, opti, epoch):
    ...
    for batch_idx, (real_img, labels) in enumerate(trainloader):   
        ...
        for i in range(len(noisy_img)):
            noisy_img[i] = add_noise(noisy_img[i])

        # direct spike input
        spike_input = noisy_img.unsqueeze(-1).repeat(1, 1, 1, 1, n_steps) # (N,C,H,W,T)
        ...
        if batch_idx > 4:
            ...
            torchvision.utils.save_image((noisy_img+1)/2, f'checkpoint/{args.name}/imgs/train/epoch{epoch}_input.png')
    ...
    with torch.no_grad():
        for batch_idx, (real_img, labels) in enumerate(testloader):   
            ...
            if batch_idx > 4:
                ...
                torchvision.utils.save_image((noisy_img+1)/2, f'checkpoint/{args.name}/imgs/test/epoch{epoch}_input.png')
